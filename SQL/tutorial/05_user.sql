...
FROM users where balance>=(select avg(balance) from users)

--지역번호가 02인 사람 조회
SELECT last_name, first_name FROM users where phone like "02%";
-- 나이 내림차순으로 10명 조회
SELECT first_name FROM users ORDER BY age desc LIMIT 10;
SELECT last_name,first_name FROM users ORDER BY age,last_name desc LIMIT 5;